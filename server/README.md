...
![app](https://github.com/htmw/EasyWay/blob/main/server/data/helpers/custom_app.PNG)
...
![models](https://github.com/htmw/EasyWay/blob/main/server/data/helpers/model_config.PNG)

Initialize and run the Flask app on port 3600 of your local machine by running the following command from the root directory of this repo in a command prompt or shell.
```bash
python app.py
```
...
### Detections API (http://localhost:3600/detections)
...
![response](https://github.com/htmw/EasyWay/blob/main/server/data/helpers/detections_api_response.PNG)
...
```bash
curl.exe -X POST -F images=@data/images/dog.jpg "http://localhost:3600/detections"
```
The JSON response should be outputted to the commmand prompt if it worked successfully.

### Image API (http://localhost:3600/image)
...
To access and test the API through Curl, open a second command prompt or shell (may have to run as Administrator). Then cd your way to the root folder of this repository (Object-Detection-API) and run the following command.
```bash
curl.exe -X POST -F images=@data/images/dog.jpg "http://localhost:3600/image" --output test.png
```
